...
function starterTemplateRoot(): string {
  ...
  return fileURLToPath(
    new URL("../../templates/starter/", import.meta.url),
  );
}

export async function initializeStarter(
  requestedDirectory = DEFAULT_STARTER_DIRECTORY,
  ...
  if (
    ...
    throw new Error(
      "init target must be a non-empty relative directory within the current working directory",
    );
  }
  ...
  if (!isWithin(canonicalWorkingDirectory, target)) {
    throw new Error(
      "init target must remain within the current working directory",
    );
  }
  ...
  if (
    canonicalParent !== canonicalWorkingDirectory &&
    !isWithin(canonicalWorkingDirectory, canonicalParent)
  ) {
    throw new Error(
      "init target parent must remain within the current working directory",
    );
  }
