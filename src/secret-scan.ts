...
export type SecretArtifactContent =
  | string
  | Uint8Array
  | readonly Uint8Array[];
...
export const DEFAULT_SECRET_SCAN_LIMITS: Readonly<SecretScanLimits> =
  Object.freeze({
    maxArtifacts: 1_000,
...
export const HARD_SECRET_SCAN_LIMITS: Readonly<SecretScanLimits> =
  Object.freeze({
    maxArtifacts: 2_000,
    maxArtifactBytes: 20 * 1024 * 1024,
...
const STATIC_RULES: readonly StaticRule[] = Object.freeze([
  {
    id: "private-key",
    pattern:
  ...
  },
  {
    id: "github-token",
    pattern:
      /\b(?:gh[pousr]_[A-Za-z0-9]{36,255}|github_pat_[A-Za-z0-9_]{20,255})\b/g,
  },
  {
    id: "gitlab-token",
  ...
  },
  {
    id: "sendgrid-api-key",
    pattern:
      /\bSG\.[A-Za-z0-9_-]{16,256}\.[A-Za-z0-9_-]{20,256}\b/g,
  },
  {
    id: "npm-access-token",
  ...
  },
  {
    id: "json-web-token",
    pattern:
      /\beyJ[A-Za-z0-9_-]{5,512}\.[A-Za-z0-9_-]{8,2048}\.[A-Za-z0-9_-]{8,2048}\b/g,
  },
  {
    id: "authorization-header",
    pattern:
      /\bauthorization\s*[:=]\s*["']?(?:bearer|basic)\s+[A-Za-z0-9._~+/=-]{12,512}/gi,
  },
]);

const CREDENTIAL_ASSIGNMENT_PATTERN =
  /(?:^|[\s,{;.\[])["']?(?:api[_-]?key|access[_-]?key|access[_-]?token|auth[_-]?token|authorization|client[_-]?secret|credential|credentials|password|passwd|pwd|secret|token|private[_-]?key|aws[_-]?secret[_-]?access[_-]?key|account[_-]?key|_auth[_-]?token)["']?\]?\s*(?::|=)\s*(?:"((?:\\.|[^"\\]){0,2048})"|'((?:\\.|[^'\\]){0,2048})'|([\[{]|[^\s,;}\]"'\[{]{1,2048}))/gim;

const CREDENTIALED_URL_PATTERN =
  /\b[a-z][a-z0-9+.-]{1,20}:\/\/[^\s/:@]{1,128}:([^\s/@]{4,256})@[^\s/"']+/gi;
...
function resolveLimits(options: SecretScanOptions): SecretScanLimits {
  if (
    !isRecord(options) ||
    ...
    throw new SecretScanError(
      "invalid_limits",
      "secret scan options contain unsupported fields",
    );
  }
  ...
  for (const key of LIMIT_KEYS) {
    ...
    if (
      ...
      throw new SecretScanError(
        "invalid_limits",
        `secret scan ${key} must be a positive integer within its hard cap`,
      );
    }
    limits[key] = value;
  }
...
function validatePath(path: unknown, artifactIndex: number): string {
  if (
    typeof path !== "string" ||
    path.length === 0 ||
    path.length > 512 ||
    ...
    throw new SecretScanError(
      "invalid_input",
      `secret scan artifact ${artifactIndex} has an invalid logical path`,
    );
  }
  return path;
}

function validateField(field: unknown, artifactIndex: number): string | undefined {
  if (field === undefined) {
    return undefined;
  }
  if (
    ...
    throw new SecretScanError(
      "invalid_input",
      `secret scan artifact ${artifactIndex} has an invalid logical field`,
    );
  }
  return field;
}
...
function assertWithinArtifactLimit(
  bytes: number,
  ...
  if (bytes > limits.maxArtifactBytes) {
    throw new SecretScanError(
      "artifact_size_exceeded",
      `secret scan artifact ${artifactIndex} exceeds the configured byte limit`,
    );
  }
}
...
function readArtifactBytes(
  ...
  limits: SecretScanLimits,
  artifactIndex: number,
): { bytes: Buffer; suppliedAsText: boolean } {
  if (typeof content === "string") {
    if (containsUnpairedSurrogate(content)) {
      throw new SecretScanError(
        "invalid_input",
        `secret scan artifact ${artifactIndex} contains invalid text`,
      );
    }
  ...
  if (Array.isArray(content)) {
    ...
    for (const chunk of content as unknown[]) {
      if (!(chunk instanceof Uint8Array)) {
        throw new SecretScanError(
          "invalid_input",
          `secret scan artifact ${artifactIndex} contains an invalid byte chunk`,
        );
      }
  ...
  throw new SecretScanError(
    "invalid_input",
    `secret scan artifact ${artifactIndex} has invalid content`,
  );
}
...
function decodeForScanning(
  bytes: Uint8Array,
  suppliedAsText: boolean,
): { text: string; lineNumbersAvailable: boolean } {
  if (suppliedAsText) {
...
function isPlaceholderOrReference(
  ...
  return (
    ...
    /^\$\{[A-Za-z_][A-Za-z0-9_]*\}$/.test(value) ||
    /^\{\{\s*(?:secrets?|env|vars)\.[A-Za-z_][A-Za-z0-9_.-]*\s*\}\}$/i.test(
      value,
    ) ||
    /^<(?:redacted|masked|placeholder|api[_-]?key|token|password|secret|credential)>$/i.test(
      value,
    ) ||
    (!quoted &&
      (/^(?:[A-Za-z_$][\w$]*\.)*(?:env|credentials?|secrets?|tokens?)(?:\.[A-Za-z_$][\w$]*)+$/i.test(
        value,
      ) ||
        /^(?:getenv|Deno\.env\.get)\((?:["'])?[A-Za-z_][A-Za-z0-9_]*(?:["'])?\)$/i.test(
          value,
        ) ||
        /^os\.environ\[(?:["'])[A-Za-z_][A-Za-z0-9_]*(?:["'])\]$/i.test(
          value,
        ))) ||
    /^(?:redacted|masked|placeholder|example|sample|dummy|changeme|change-me|password|not[_-]?a[_-]?secret|none|null|undefined|test|x{8,}|\*{8,})$/i.test(
      lower,
    ) ||
...
function metadataContainsSecretShape(value: string): boolean {
  ...
  for (const match of value.matchAll(assignmentPattern)) {
    ...
    if (
      !isPlaceholderOrReference(
        assignedValue,
        match[3] !== undefined && (assignedValue === "{" || assignedValue === "["),
  ...
  const urlPattern = new RegExp(
    CREDENTIALED_URL_PATTERN.source,
    CREDENTIALED_URL_PATTERN.flags,
  );
...
function compareFindings(
  left: SecretScanFinding,
  right: SecretScanFinding,
): number {
  return (
 ...
 */
export function scanNamedArtifactsForSecrets(
  artifacts: readonly NamedSecretArtifact[],
  ...
  if (!Array.isArray(artifacts) || artifacts.length === 0) {
    throw new SecretScanError(
      "invalid_input",
      "secret scan requires a non-empty artifact set",
    );
  }
  if (artifacts.length > limits.maxArtifacts) {
    throw new SecretScanError(
      "artifact_limit_exceeded",
      "secret scan artifact count exceeds the configured limit",
    );
  }
  ...
  const normalized = artifacts.map((artifact, index) => {
    if (!isRecord(artifact)) {
      throw new SecretScanError(
        "invalid_input",
        `secret scan artifact ${index} is invalid`,
      );
    }
    if (
      Object.keys(artifact).some(
        (key) => key !== "path" && key !== "field" && key !== "content",
      )
    ) {
      throw new SecretScanError(
        "invalid_input",
        `secret scan artifact ${index} contains unsupported fields`,
      );
    }
  ...
  for (const artifact of normalized) {
    ...
    if (seenLocations.has(key)) {
      throw new SecretScanError(
        "invalid_input",
        "secret scan artifact locations must be unique",
      );
    }
    seenLocations.add(key);
  }

  const findings = new Map<string, SecretScanFinding>();
  let findingsTruncated = false;
  let scannedBytes = 0;

  const addFinding = (finding: Omit<SecretScanFinding, "findingDigest">): void => {
    ...
    const safeFinding: SecretScanFinding = Object.freeze({
      ...finding,
  ...
  for (const artifact of normalized) {
    const { bytes, suppliedAsText } = readArtifactBytes(
      artifact.content,
      limits,
      artifact.originalIndex,
    );
    if (bytes.byteLength > limits.maxTotalBytes - scannedBytes) {
      bytes.fill(0);
      throw new SecretScanError(
        "total_size_exceeded",
        "secret scan total bytes exceed the configured limit",
      );
    }
    scannedBytes += bytes.byteLength;
    try {
      const { text, lineNumbersAvailable } = decodeForScanning(
        bytes,
        suppliedAsText,
      );
      ...
      const locationFor = (
        ruleId: SecretScanRuleId,
        index: number,
      ): Omit<SecretScanFinding, "findingDigest"> => ({
        ruleId,
        path: artifact.path,
      ...
      for (const match of text.matchAll(assignmentPattern)) {
        ...
        if (
          ...
          const assignmentIndex =
            firstCharacter !== undefined && /[\s,{;]/.test(firstCharacter)
              ? match.index + 1
              : match.index;
          addFinding(
            locationFor("suspicious-credential-assignment", assignmentIndex),
          );
        }
      }
  ...
  const stableFindings = Object.freeze(
 ...
 */
export function assertSecretScanClean(
  ...
  throw new SecretScanError(
    "findings_detected",
    `${boundary} failed static secret scanning: ${[
      ...safeFindings,
      suffix,
    ].join("; ")}`,
  );
