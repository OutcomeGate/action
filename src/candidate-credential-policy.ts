...
const CANDIDATE_RUNTIME_ENV_NAME_SET = new Set<string>(
  CANDIDATE_RUNTIME_ENV_NAMES,
);

export type CandidateCredentialPolicyErrorCode =
  | "adapter_boundary_invalid"
  | "adapter_name_overlap"
  | "adapter_value_overlap"
  | "allowlist_mismatch"
  | "candidate_value_overlap"
  | "duplicate_env_name"
  ...
  | "reserved_env_name"
  | "runtime_name_not_allowed"
  | "runtime_name_overlap"
  | "runtime_value_overlap";
...
export type CandidateCredentialPolicy =
  | NoCandidateCredentialsPolicy
  | ExplicitCandidateCredentialsPolicy;
...
function validateCredentialEnvName(name: unknown): asserts name is string {
  if (
    typeof name !== "string" ||
    ...
    fail(
      "invalid_env_name",
      "candidate credential declaration contains an invalid environment name",
    );
  }
  if (isReservedCredentialEnvironmentName(name)) {
    fail(
      "reserved_env_name",
      `candidate credential environment name '${name}' is reserved`,
    );
  }
}
...
function validateNameList(
  ...
  for (const candidate of value) {
    ...
    if (seen.has(candidate)) {
      fail(
        "duplicate_env_name",
        `${source} repeats candidate credential environment name '${candidate}'`,
      );
    }
    seen.add(candidate);
...
export function parseCandidateCredentialPolicy(
  value: unknown,
): CandidateCredentialPolicy {
  if (!isRecord(value)) {
  ...
  if (kind === "none") {
    if (!exactOwnStringKeys(value, ["kind"])) {
      fail(
        "invalid_policy",
        "candidate no-credentials policy contains unsupported fields",
      );
    }
  ...
  if (kind === "environment") {
    if (!exactOwnStringKeys(value, ["kind", "environment"])) {
      fail(
        "invalid_policy",
        "candidate environment-credential policy contains unsupported fields",
      );
    }
    ...
    if (names.length === 0) {
      fail(
        "invalid_policy",
        "an empty candidate credential grant must use the explicit 'none' policy",
      );
    }
  ...
  fail(
    "invalid_policy",
    "candidate credential policy kind must be 'none' or 'environment'",
  );
}
...
function validateBoundedValue(
  value: unknown,
  emptyCode: CandidateCredentialPolicyErrorCode,
  invalidCode: CandidateCredentialPolicyErrorCode,
  subject: string,
): string {
...
function validateAdapterBoundary(
  boundary: AdapterCredentialBoundary,
): Readonly<Record<string, string>> {
  if (!isRecord(boundary)) {
    fail("adapter_boundary_invalid", "adapter credential boundary is invalid");
  }

  let declaredInput: unknown;
  let environmentInput: unknown;
  try {
  ...
  const environmentKeys = readObjectKeys(
    environmentInput,
    "adapter_boundary_invalid",
    "adapter credential boundary is invalid",
  );
  ...
  for (const name of declared) {
    ...
    let value: string;
    try {
      value = validateBoundedValue(
        observed.value,
        "adapter_boundary_invalid",
        "adapter_boundary_invalid",
        "adapter credential value",
      );
    } catch {
      fail("adapter_boundary_invalid", "adapter credential boundary is invalid");
    }
...
function validateRuntimeEnvironment(
  ...
  const names = readObjectKeys(
    value,
    "invalid_runtime_environment",
    "non-secret runtime environment is invalid",
  );
  ...
  for (const name of names) {
    if (!CANDIDATE_RUNTIME_ENV_NAME_SET.has(name)) {
      fail(
        "runtime_name_not_allowed",
        `candidate runtime environment name '${name}' is not allowed`,
      );
    }
    ...
    runtime[name] = validateBoundedValue(
      observed.value,
      "invalid_runtime_environment",
      "invalid_runtime_environment",
      `candidate runtime environment value '${name}'`,
    );
  }
...
function assertDisjointValues(options: {
  ...
  adapter: Readonly<Record<string, string>>;
  ...
  for (let index = 0; index < candidateEntries.length; index += 1) {
    ...
    for (const adapterValue of adapterValues) {
      if (valuesOverlap(candidateValue, adapterValue)) {
        fail(
          "adapter_value_overlap",
          "candidate and adapter credential values overlap",
        );
      }
    }
    for (let otherIndex = index + 1; otherIndex < candidateEntries.length; otherIndex += 1) {
      ...
      if (other !== undefined && valuesOverlap(candidateValue, other[1])) {
        fail(
          "candidate_value_overlap",
          "candidate credential values overlap each other",
        );
      }
    }
    for (const [, runtimeValue] of runtimeEntries) {
      if (runtimeValue.includes(candidateValue)) {
        fail(
          "runtime_value_overlap",
          "candidate credential and runtime values overlap",
        );
      }
    }
  }
  ...
  for (const adapterValue of adapterValues) {
    for (const [, runtimeValue] of runtimeEntries) {
      if (runtimeValue.includes(adapterValue)) {
        fail(
          "adapter_value_overlap",
          "adapter credential and candidate runtime values overlap",
        );
      }
    }
  }
 ...
 */
export function authorizeCandidateEnvironment(
  options: CandidateEnvironmentAuthorization,
): Readonly<Record<string, string>> {
  const policy = parseCandidateCredentialPolicy(options.credentialPolicy);
  const callerAllowlist = validateNameList(
  ...
  if (!setsEqual(declared, callerAllowlist)) {
    fail(
      "allowlist_mismatch",
      "caller allowlist must exactly match the candidate credential declaration",
    );
  }
  ...
  const runtimeEnvironment = validateRuntimeEnvironment(
    options.nonSecretRuntimeEnvironment,
  );
  ...
  for (const name of declared) {
    if (adapterNames.has(name)) {
      fail(
        "adapter_name_overlap",
        `candidate credential environment name '${name}' is already used by the adapter`,
      );
    }
  }
  for (const name of Object.keys(runtimeEnvironment)) {
    if (adapterNames.has(name)) {
      fail(
        "runtime_name_overlap",
        `candidate runtime environment name '${name}' is already used by the adapter`,
      );
    }
  }
  ...
  if (declared.length > 0) {
    if (
      options.capturedReleaseDigest === undefined ||
      options.approvedReleaseDigest === undefined
    ) {
      fail(
        "release_digest_required",
        "candidate credentials require captured and independently approved release digests",
      );
    }
    if (
      !SHA256_PATTERN.test(options.capturedReleaseDigest) ||
      !SHA256_PATTERN.test(options.approvedReleaseDigest)
    ) {
      fail(
        "invalid_release_digest",
        "candidate credential release digests must be exact lowercase SHA-256 digests",
      );
    }
    if (options.capturedReleaseDigest !== options.approvedReleaseDigest) {
      fail(
        "release_digest_mismatch",
        "approved candidate release digest does not match the captured release",
      );
    }
    ...
    for (const name of declared) {
      ...
      if (!observed.present || typeof observed.value !== "string") {
        fail(
          "missing_credential_value",
          `candidate credential environment value '${name}' is missing`,
        );
      }
      candidateEnvironment[name] = validateBoundedValue(
        observed.value,
        "empty_credential_value",
        "invalid_credential_value",
        `candidate credential environment value '${name}'`,
      );
    }
  }
