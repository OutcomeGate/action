...
import {
  ...
  writeJsonReport,
  writeSanitizedPublicationReport,
  writeTextFile,
} from "./report.js";
import {
...
function credentialNamesFromArgv(argv: readonly string[]): string[] {
  const names: string[] = [];
  for (let index = 0; index < argv.length; index += 1) {
    const argument = argv[index]!;
    const equals = [...CREDENTIAL_VALUE_OPTIONS].find((option) =>
...
const invokedCredentialSecrets: readonly KnownSecret[] = Object.freeze(
  invokedCredentialNames.flatMap((name, index) => {
...
function invocationCredentialSource(
...
let useFixedActionDiagnostics =
  process.argv.includes("--github") || invokedCredentialNames.length > 0;
...
function renderGithubAnnotations(
  report: ReleaseReport,
  assertSemanticSafe: (value: string) => void,
): string {
...
function assertNoInvokedCredentialLeaks(content: string): void {
  assertNoKnownSecretLeaks(content, invokedCredentialSecrets);
}

function writeScannedStdout(content: string, boundary: string): void {
  assertNoInvokedCredentialLeaks(content);
  assertSecretScanClean(
    scanTextForSecrets({ path: "cli/stdout.txt", text: content }),
    boundary,
  );
  process.stdout.write(content);
}

function writeScannedStderr(content: string, boundary: string): void {
  assertNoInvokedCredentialLeaks(content);
  assertSecretScanClean(
    scanTextForSecrets({ path: "cli/stderr.txt", text: content }),
    boundary,
  );
  process.stderr.write(content);
}

function publishableErrorMessage(error: unknown): string {
  const message = error instanceof Error ? error.message : String(error);
  try {
    assertSecretScanClean(
      scanTextForSecrets({ path: "cli/error.txt", text: message }),
      "CLI diagnostics",
    );
    return message;
  } catch {
...
async function main(): Promise<number> {
  ...
  if (
    ...
    throw new Error(
      "adapter credential options require --adapter-manifest",
    );
  }
  const adapterOptions = {
    ...
    ...(parsed.values["allow-adapter-env"] !== undefined
      ? {
          ...
          sourceEnv: invocationCredentialSource(
            parsed.values["allow-adapter-env"],
          ),
        }
      : {}),
    ...(parsed.values["approved-adapter-digest"] !== undefined
      ? {
          approvedAdapterDigest:
            parsed.values["approved-adapter-digest"],
        }
      : {}),
  };
  if (command === "validate") {
    ...
    const issues = await validateSuiteAgainstAdapter(
      loaded.suite,
      adapter.definition,
    );
    ...
    writeScannedStdout(
      `valid ${loaded.suite.name}@${loaded.suite.version}: ${loaded.suite.scenarios.length} scenario(s), adapter contract ${adapter.definition.id}@${adapter.definition.version}\n`,
  ...
  if (command === "validate-release") {
    ...
    writeScannedStdout(
      `valid ${capture.identity.name}: ${capture.identity.fileCount} file(s), release ${capture.identity.releaseDigest}\n`,
  ...
  if (command === "validate-adapter") {
    ...
    if (
      ...
      throw new Error(
        "validate-adapter uses --manifest, not --adapter or --adapter-manifest",
      );
    }
    const capture = await loadAdapterManifest(parsed.values.manifest);
    writeScannedStdout(
      `valid declared adapter ${capture.identity.id}@${capture.identity.version}: ${capture.identity.fileCount} file(s), adapter ${capture.identity.adapterDigest} (not executed)\n`,
  ...
  if (command === "adapter-check") {
    if (
      ...
      throw new Error(
        "adapter-check requires exactly one of --adapter-manifest or --adapter",
      );
    }
    const adapter =
      parsed.values["adapter-manifest"] !== undefined
    ...
    writeScannedStdout(
      `valid adapter ${adapter.definition.id}@${adapter.definition.version}: ${adapter.definition.conformance.length} conformance case(s)\n`,
  ...
  if (command === "check") {
    ...
    const common = {
      ...
      ...(parsed.values["allow-adapter-env"] !== undefined
        ? {
            adapterSourceEnv: invocationCredentialSource(
              parsed.values["allow-adapter-env"],
            ),
          }
        : {}),
      ...(parsed.values["allow-candidate-env"] !== undefined
        ? {
            candidateCallerAllowlist:
              parsed.values["allow-candidate-env"],
            candidateSourceEnv: invocationCredentialSource(
              parsed.values["allow-candidate-env"],
            ),
          }
        : {}),
      ...(parsed.values["approved-release-digest"] !== undefined
        ? {
            approvedReleaseDigest:
              parsed.values["approved-release-digest"],
          }
        : {}),
      ...(parsed.values["require-explicit-candidate-policy"] === true
    ...
    if (
      ...
      if (parsed.values["publication-report"] !== undefined) {
        ...
        await writeSanitizedPublicationReport(
          parsed.values["publication-report"],
          publication,
        );
      }
      if (parsed.values["publication-markdown"] !== undefined) {
        ...
        await writeTextFile(
          parsed.values["publication-markdown"],
          publicationMarkdown,
        );
      }
    }
...
main()
  ...
  .catch((error: unknown) => {
    const diagnostic = `INDETERMINATE ${
        useFixedActionDiagnostics
