...
export type AdapterHostClientErrorCode =
  | "cancelled"
  | "containment_failed"
  | "host_crashed"
...
type ScenarioState = "uninitialized" | "active" | "closed";

interface PendingRequest {
  seq: number;
  expectedType: AdapterHostResponse["type"];
  deadlineAt: number;
  timer: NodeJS.Timeout;
  resolve: (response: AdapterHostResponse) => void;
  reject: (error: Error) => void;
}

type AdapterHostRequestInput = AdapterHostRequest extends infer Request
  ? Request extends AdapterHostRequest
    ? Omit<Request, "v" | "nonce" | "seq">
    : never
  : never;
...
function boundedTimeout(value: number | undefined, fallback: number): number {
  ...
  if (
    ...
    throw new AdapterHostClientError(
      "invalid_state",
      `adapter-host timeout must be from 1 through ${MAX_OPERATION_TIMEOUT_MS}ms`,
    );
  }
  return selected;
}
...
function scanForSecrets(
  ...
  try {
    assertNoKnownSecretLeaksAtJsonBoundary(
      value as Parameters<typeof assertNoKnownSecretLeaksAtJsonBoundary>[0],
      knownSecrets,
    );
  } catch {
    throw new AdapterHostClientError(
      "secret_leak",
      "known credential material attempted to cross the adapter-host protocol",
    );
  }
}
...
export class AdapterHostClient {
  ...
  private readonly knownSecrets: readonly KnownSecret[];
  private readonly readyDeferred = deferred<void>();
  private readonly processClosedDeferred = deferred<AdapterHostExit>();
  private readonly terminalListeners = new Set<
    (error: AdapterHostClientError) => void
  >();
  private state: ClientState = "starting";
  private scenarioState: ScenarioState = "uninitialized";
  private sequence = 0;
  private pending: PendingRequest | undefined;
  private descriptorValue: AdapterDescriptorV2 | undefined;
  private moduleDigestValue: string | undefined;
  private terminalError: AdapterHostClientError | undefined;
  private exitValue: AdapterHostExit | undefined;
  private expectedNormalExit = false;
  private terminationPromise: Promise<void> | undefined;
  private stdoutBytes: Buffer = Buffer.alloc(0);
  ...
  static async spawn(options: AdapterHostClientOptions): Promise<AdapterHostClient> {
    ...
    if (!isProtocolPayload(options.target)) {
      throw new AdapterHostClientError(
        "protocol_error",
        "adapter target must be finite plain JSON within the protocol limit",
      );
    }

    const adapterPath = resolve(options.adapterPath);
    const expectedModuleDigest =
      options.expectedModuleDigest ?? sha256(await readFile(adapterPath));
    if (!/^[a-f0-9]{64}$/.test(expectedModuleDigest)) {
      throw new AdapterHostClientError(
        "identity_mismatch",
        "expected adapter module digest must be lowercase SHA-256",
      );
    }
    ...
    const protectedSecrets: readonly KnownSecret[] = Object.freeze(
      (options.protectedSecrets ?? []).map((secret, index) =>
        Object.freeze({
    ...
    const knownSecrets: readonly KnownSecret[] = Object.freeze([
      ...adapterSecrets,
      ...protectedSecrets,
    ]);
    ...
    const hostPath =
      options.hostPath ?? fileURLToPath(new URL("./main.js", import.meta.url));
    ...
    try {
      child = fork(
        hostPath,
        [options.mode, adapterPath, nonce, credentialNamesArgument],
        {
          cwd: hostWorkingDirectory,
      ...
      throw new AdapterHostClientError(
        "spawn_failed",
        `adapter host could not be spawned: ${cleanReason(error)}`,
      );
    }
  ...
  get descriptor(): AdapterDescriptorV2 {
    if (this.descriptorValue === undefined) {
      throw new AdapterHostClientError(
        "invalid_state",
        "adapter host has not completed its handshake",
      );
    }
    ...
    return (await this.request(
      {
        ...
        pointers: cloneProtocolJson(
          options.pointers as unknown as ProtocolJsonValue,
        ) as unknown as PointerValidationRequest[],
      },
      "validation_result",
      timeoutMs,
  ...
  async close(timeoutMs?: number): Promise<void> {
    ...
    if (exit === undefined) {
      const error = new AdapterHostClientError(
        "timeout",
        "adapter host acknowledged close but did not exit before its deadline",
      );
    ...
    if (exit.code !== 0 || exit.signal !== null) {
      const error = new AdapterHostClientError(
        "host_crashed",
        `adapter host exited after close with code ${String(exit.code)} and signal ${String(exit.signal)}`,
      );
  ...
  async cancel(reason = "adapter-host operation cancelled"): Promise<void> {
  ...
  private attachChildListeners(): void {
    ...
    this.child.once("error", (error) => {
      void this.poison(
        new AdapterHostClientError(
          "spawn_failed",
          `adapter host process error: ${cleanReason(error)}`,
        ),
      );
    });
    this.child.once("exit", (code, signal) => {
      ...
      if (
        ...
        void this.poison(
          new AdapterHostClientError(
            "host_crashed",
            `adapter host exited unexpectedly with code ${String(code)} and signal ${String(signal)}`,
          ),
        );
      }
    });
  ...
  private consumeDiagnostic(
    ...
    let combined = `${
      stream === "stdout" ? this.stdoutSecretTail : this.stderrSecretTail
    }${decoded}`;
    let leaked = false;
    ...
    const longestSecret = this.knownSecrets.reduce(
      (longest, secret) => Math.max(longest, secret.value.length),
      0,
    );
    const retainedCharacters = final
      ? 0
      : Math.min(
          Math.max(0, longestSecret - 1),
    ...
    if (leaked) {
      void this.poison(
        new AdapterHostClientError(
          "secret_leak",
          `known credential material appeared in adapter ${stream} diagnostics`,
        ),
      );
    }
  }
  ...
  private async waitForReady(timeoutMs: number): Promise<void> {
    ...
    try {
        ...
        new Promise<void>((_resolve, reject) => {
          timer = setTimeout(() => {
            reject(
              new AdapterHostClientError(
                "timeout",
                `adapter host did not become ready within ${timeoutMs}ms`,
              ),
            );
          }, timeoutMs);
        }),
      ...
      const failure =
        error instanceof AdapterHostClientError
          ? error
  ...
  private handleMessage(raw: unknown): void {
    ...
    let response: AdapterHostResponse | undefined;
    try {
      if (!isProtocolPayload(raw)) {
        throw new AdapterHostClientError(
          "protocol_error",
          "adapter host sent a non-JSON or oversized response",
        );
      }
      ...
      if (response === undefined) {
        throw new AdapterHostClientError(
          "protocol_error",
          "adapter host sent an invalid response",
        );
      }
      if (response.nonce !== this.nonce) {
        throw new AdapterHostClientError(
          "protocol_error",
          "adapter host response nonce did not match",
        );
      }
    } catch (error) {
      void this.poison(
    ...
    if (this.state === "starting") {
      if (response.type === "fatal") {
        const error = new AdapterHostClientError(
          "host_operation_failed",
          `adapter host ${response.phase} failed: ${response.message}`,
        );
      ...
      if (
        ...
        const error = new AdapterHostClientError(
          "protocol_error",
          "adapter host did not begin with the expected ready response",
        );
        this.readyDeferred.reject(error);
        void this.poison(error);
        return;
      ...
      if (response.moduleDigest !== this.expectedModuleDigest) {
        const error = new AdapterHostClientError(
          "identity_mismatch",
          "adapter module digest changed between capture and host import",
        );
        this.readyDeferred.reject(error);
        void this.poison(error);
        return;
    ...
    if (pending === undefined) {
      void this.poison(
        new AdapterHostClientError(
          "protocol_error",
          "adapter host sent an unsolicited or late response",
        ),
      );
      return;
    }
    if (performance.now() >= pending.deadlineAt) {
      void this.poison(
        new AdapterHostClientError(
          "timeout",
          "adapter host response lost the operation deadline race",
        ),
      );
      return;
    }
    if (response.seq !== pending.seq) {
      void this.poison(
        new AdapterHostClientError(
          "protocol_error",
          "adapter host response sequence did not match the pending request",
        ),
      );
      return;
    }
    ...
    if (response.type !== pending.expectedType) {
      void this.poison(
        new AdapterHostClientError(
          "protocol_error",
          `adapter host returned ${response.type} while ${pending.expectedType} was pending`,
        ),
      );
      return;
    }
  ...
  private async request(
    partial: AdapterHostRequestInput,
    expectedType: AdapterHostResponse["type"],
    timeoutMs: number,
    ...
    const request = {
      ...
      seq,
      ...partial,
    } as AdapterHostRequest;
    if (!isProtocolPayload(request)) {
      throw new AdapterHostClientError(
        "protocol_error",
        "adapter-host request exceeded the protocol boundary",
      );
    }
    scanForSecrets(request, this.knownSecrets);
    this.state = "pending";
    return await new Promise<AdapterHostResponse>((resolveRequest, rejectRequest) => {
      const deadlineAt = performance.now() + timeoutMs;
      const timer = setTimeout(() => {
        void this.poison(
          new AdapterHostClientError(
            "timeout",
            `${partial.type} timed out after ${timeoutMs}ms`,
          ),
        );
      }, timeoutMs);
      this.pending = {
      ...
      if (!this.child.connected) {
        void this.poison(
          new AdapterHostClientError(
            "host_crashed",
            "adapter-host IPC channel closed before the request was sent",
          ),
        );
        return;
      }
        ...
        this.child.send(request, (error) => {
          if (error !== null) {
            void this.poison(
              new AdapterHostClientError(
                "host_crashed",
                `adapter-host request could not be sent: ${cleanReason(error)}`,
              ),
            );
          }
        });
  ...
  private async poison(error: AdapterHostClientError): Promise<void> {
  ...
  private recordTerminal(error: AdapterHostClientError): void {
  ...
  private async terminateProcess(
    ...
    this.terminationPromise = (async () => {
      ...
      if ((await this.waitForExit(KILL_GRACE_MS)) === undefined) {
        throw new AdapterHostClientError(
          "containment_failed",
          "adapter host did not exit after process-group SIGKILL",
        );
      }
      this.killProcessGroup("SIGKILL");
    })();
  ...
  private async waitForExit(timeoutMs: number): Promise<AdapterHostExit | undefined> {
    let timer: NodeJS.Timeout | undefined;
    try {
  ...
  private requireMode(expected: AdapterHostMode, operation: string): void {
  ...
  private requireActiveScenario(operation: string): void {
  ...
  private requireIdle(operation: string): void {
    if (this.state !== "idle") {
      ...
      throw this.invalidState(
        `${operation} requires an idle adapter host; current state is ${this.state}`,
      );
    }
  }

  private invalidState(message: string): AdapterHostClientError {
    return new AdapterHostClientError("invalid_state", message);
  }
}

export async function spawnAdapterHost(
  options: AdapterHostClientOptions,
