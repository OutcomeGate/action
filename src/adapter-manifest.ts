...
function readConfiguration(
  ...
  return cloneJson(value) as AdapterConfiguration;
}
...
function readUniqueStrings(options: {
  ...
  path: string;
  allowEmpty: boolean;
  validate?: (value: string, path: string, issues: string[]) => boolean;
  issues: string[];
}): string[] {
  if (
    ...
    options.issues.push(
      `${options.path} must be ${options.allowEmpty ? "an" : "a non-empty"} array of strings`,
    );
    return [];
  }
  const values: string[] = [];
...
function parseTarget(
  ...
  if (value.kind === "remote") {
    unknownKeys(
      value,
      ["kind", "endpoint", "tenant", "apiVersion", "configuration"],
      "target",
      issues,
    );
...
function validEnvironmentName(
  ...
  if (
    isReservedCredentialEnvironmentName(value)
  ) {
...
export function parseAdapterManifest(value: unknown): AdapterManifestSpec {
  const issues: string[] = [];
  if (!isRecord(value)) {
  ...
  let runtime: AdapterManifestSpec["runtime"] | undefined;
  if (!isRecord(value.runtime)) {
    ...
    const operationTimeoutMs = readBoundedInteger(
      value.runtime.operationTimeoutMs,
    ...
    const shutdownTimeoutMs = readBoundedInteger(
      value.runtime.shutdownTimeoutMs,
  ...
  let bundle: AdapterManifestSpec["bundle"] | undefined;
  ...
  let contract: AdapterManifestSpec["contract"] | undefined;
  ...
  let credentials: AdapterManifestSpec["credentials"] | undefined;
...
export async function loadAdapterManifest(
  path: string,
): Promise<AdapterManifestCapture> {
  ...
  try {
    ...
    assertSecretScanClean(
      scanTextForSecrets({ path: "adapter/manifest.json", text: raw }),
      "adapter manifest",
    );
  ...
  if (normalizedJsonFiles.length > 0) {
    assertSecretScanClean(
      ...
      "normalized adapter JSON bundle files",
    );
  }
...
export async function materializeAdapter(
  capture: AdapterManifestCapture,
): Promise<MaterializedAdapter> {
...
export async function verifyMaterializedAdapter(
  ...
  try {
    ...
    return [
      `materialized adapter bundle could not be verified: ${
        error instanceof Error ? error.message : String(error)
      }`,
    ];
  }
...
export async function cleanupMaterializedAdapter(
  materialized: MaterializedAdapter,
): Promise<void> {
