...
function fail(createError, issues) {
...
export function bundleFileIdentities(files) {
...
export function computeBundleDigest(domain, files) {
    return digestValue({ domain, files: bundleFileIdentities(files) });
}
async function captureBundleOnce(root, createError) {
    let rootMetadata;
    try {
        ...
        fail(createError, [
            `bundle.root could not be inspected: ${error instanceof Error ? error.message : String(error)}`,
        ]);
    }
    ...
    await walk(root, "");
    return captured.sort((left, right) => compareCanonicalText(left.path, right.path));
}
async function assertRootPathHasNoSymlinks(manifestDirectory, relativeRoot, createError) {
...
export async function captureDeclaredBundle(options) {
    await assertRootPathHasNoSymlinks(options.manifestDirectory, options.relativeRoot, options.createError);
    ...
    if (digestValue(bundleFileIdentities(first)) !==
...
function validateCapturedFiles(files, createError) {
    const paths = new Set();
...
export async function materializedBundleMatches(options) {
    ...
    return (digestValue(bundleFileIdentities(current)) ===
        digestValue(bundleFileIdentities(options.expectedFiles)));
}
export async function cleanupMaterializedBundle(root) {
    await rm(root, { recursive: true, force: true });
}
