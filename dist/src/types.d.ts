...
export type AssertionSpec = JsonPointerAssertion | EventCountAssertion | EventOrderAssertion;
...
export interface ReleaseManifestSpecV1 extends ReleaseManifestCommon<NoModelSpec | RemoteModelSpec> {
    schemaVersion: "agentci.release.v1";
}
export interface ReleaseManifestSpecV2 extends ReleaseManifestCommon<ReleaseModelSpec> {
    schemaVersion: "agentci.release.v2";
    candidate: {
        credentials: CandidateCredentialPolicySpec;
    };
}
export type ReleaseManifestSpec = ReleaseManifestSpecV1 | ReleaseManifestSpecV2;
...
export type AdapterTargetSpec = SyntheticAdapterTargetSpec | RemoteAdapterTargetSpec;
...
export type ReleaseIdentity = EntryFileReleaseIdentity | ManifestReleaseIdentity;
...
export interface Environment {
    ...
    transition?(request: EnvironmentTransitionRequest): EnvironmentTransitionResult | Promise<EnvironmentTransitionResult>;
...
export interface AdapterRuntime {
    ...
    validateStatePointer(pointer: string, initialState: JsonValue): string | undefined | Promise<string | undefined>;
    validate?(suite: SuiteSpec, pointers: readonly AdapterPointerValidationRequest[]): AdapterValidationBatch | Promise<AdapterValidationBatch>;
    createEnvironment(initialState: JsonValue, context?: {
        scenarioId: string;
        timeoutMs: number;
