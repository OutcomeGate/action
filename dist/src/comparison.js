...
function statusMap(report) {
    return new Map(report.scenarios.map((scenario) => [scenario.scenarioId, scenario.verdict]));
}
export function compareReports(baseline, candidate) {
    ...
    if (baseline.adapter.apiVersion !== candidate.adapter.apiVersion ||
    ...
    if (baseline.adapter.digestScope === "module-entry-only" &&
    ...
    if (baseline.adapter.digestScope ===
    ...
    if (baseline.release.digestScope === "declared-config-and-bundle-bytes" &&
    ...
    if (baselineStatuses.size !== candidateStatuses.size ||
    ...
    let verdict = candidate.decision.verdict;
    if (baseline.decision.verdict === "indeterminate" ||
