...
import { ADAPTER_HOST_PROTOCOL_VERSION, EXTERNAL_ADAPTER_API_VERSION, cloneProtocolJson, isAdapterDescriptorV2, isProtocolPayload, parseAdapterHostRequest, } from "./protocol.js";
...
let knownSecrets = Object.freeze([]);
let lastSequence = 0;
let activeOperation;
let activeTask;
...
function assertAdapterDefinition(value) {
    if (!isRecord(value)) {
    ...
    if (typeof value.validateSuite !== "function" ||
        ...
        throw new Error("adapter must provide validateSuite, validateStatePointer, and createEnvironment");
    }
    const descriptor = {
    ...
    if (!isAdapterDescriptorV2(descriptor)) {
        throw new Error(`adapter descriptor must satisfy ${EXTERNAL_ADAPTER_API_VERSION}`);
    }
    return value;
}
function assertEnvironment(value) {
    if (!isRecord(value) ||
        ...
        throw new Error("createEnvironment must return tools plus call, snapshot, and mandatory close methods");
    }
    return value;
}
function boundaryJson(value, label) {
    if (!isProtocolPayload(value)) {
        throw new HostOperationError(label, `${label} did not return finite plain JSON within the protocol limit`);
    }
...
function isExpectedToolError(value) {
...
function makeContext(operation) {
...
async function sendFrame(frame) {
    ...
    assertNoKnownSecretLeaksAtJsonBoundary(frame, knownSecrets);
...
function beginOperation(request) {
...
function ensureOperationLive(operation) {
    if (operation.timedOut) {
        throw new HostOperationError(operation.phase, `${operation.phase} exceeded its host deadline`);
    }
...
function requireState(expected, operation) {
    if (state !== expected) {
        throw new HostOperationError(operation, `${operation} is not valid while the host is ${state}`);
    }
}
async function snapshot(target, context, phase) {
    try {
...
async function handleValidate(request, operation) {
    if (mode !== "inspect") {
    ...
    if (!Array.isArray(issues) ||
        ...
        throw new HostOperationError("validateSuite", "validateSuite must return an array of non-empty strings");
    }
    const pointers = [];
    for (const pointer of request.pointers) {
        let issue;
        try {
            issue = await adapter.validateStatePointer(pointer.pointer, cloneProtocolJson(pointer.initialState), context);
        }
        catch (error) {
        ...
        if (issue !== undefined && (typeof issue !== "string" || issue.length === 0)) {
            throw new HostOperationError("validateStatePointer", "validateStatePointer must return undefined or a non-empty string");
        }
...
async function handleInitialize(request, operation) {
    if (mode !== "scenario") {
        throw new HostOperationError("initialize", "initialize requires scenario mode");
    }
    ...
    let created;
    try {
        created = await definition.createEnvironment(cloneProtocolJson(request.initialState), context);
    }
    catch (error) {
    ...
    const initialState = await snapshot(environment, context, "initialSnapshot");
    ensureOperationLive(operation);
    state = "active";
    return {
        v: ADAPTER_HOST_PROTOCOL_VERSION,
...
async function handleTransition(request, operation) {
    requireState("active", "transition");
    const target = environment;
    const context = makeContext(operation);
    const beforeState = await snapshot(target, context, "beforeSnapshot");
    ensureOperationLive(operation);
    let outcome = { kind: "skipped" };
    if (request.invoke) {
        try {
            const content = await target.call(request.tool, cloneProtocolJson(request.arguments), context);
            ensureOperationLive(operation);
...
async function handleSnapshot(request, operation) {
    requireState("active", "snapshot");
    const stateValue = await snapshot(environment, makeContext(operation), "snapshot");
    ensureOperationLive(operation);
    return {
        v: ADAPTER_HOST_PROTOCOL_VERSION,
        nonce: nonce,
...
async function handleClose(request, operation) {
    if (mode === "inspect") {
...
async function executeRequest(request, operation) {
    let response;
    try {
...
async function sendFatal(phase, error) {
    if (normalExit) {
        return;
    }
    state = "poisoned";
...
async function emergencyExit(reason) {
    ...
    if (activeTask !== undefined) {
        await Promise.race([
            ...
            new Promise((resolveDelay) => setTimeout(resolveDelay, 75)),
        ]);
    }
    if (environment !== undefined &&
        ...
        try {
            await Promise.race([
                Promise.resolve(environment.close({
                ...
                new Promise((_resolve, reject) => setTimeout(() => reject(new Error("emergency close timed out")), 75)),
            ]);
        }
        catch {
...
async function boot() {
    ...
    if (!Array.isArray(credentialNames) ||
        !credentialNames.every((name) => typeof name === "string" && /^[A-Z][A-Z0-9_]*$/.test(name)) ||
...
void boot().catch((error) => sendFatal("boot", error));
