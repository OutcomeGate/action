...
export function parseStrictJson(text) {
    ...
    if (text.length > MAX_JSON_CODE_UNITS) {
        throw new StrictJsonError("input_too_large", "JSON input exceeds the strict parser limit");
    }
    ...
    while (pending.length > 0) {
        ...
        if (typeof value === "number" && !Number.isFinite(value)) {
            throw new StrictJsonError("number_out_of_range", "JSON number is outside the finite runtime range");
        }
    ...
    const parseValue = (depth) => {
        if (depth > MAX_JSON_DEPTH) {
            throw new StrictJsonError("nesting_too_deep", "JSON input exceeds the strict nesting limit");
        }
        skipWhitespace();
        ...
        if (token === "{") {
            ...
            while (true) {
                ...
                if (members.has(member)) {
                    throw new StrictJsonError("duplicate_member", "JSON object contains a duplicate member");
                }
                members.add(member);
                skipWhitespace();
                if (text[index] !== ":") {
