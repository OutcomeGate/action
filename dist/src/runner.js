...
import { prepareManifestAdapter, resolveAdapter, runAdapterConformance, validateSuiteAgainstAdapter, } from "./adapter.js";
import { digestFile, digestNamedFiles, digestValue, isJsonValue, cloneJson, } from "./canonical.js";
import { evaluateAssertions } from "./assertions.js";
import { runCandidateProcess } from "./driver/process.js";
import { AdapterValidationError, ReleaseValidationError, SuiteValidationError, } from "./errors.js";
import { decideGate } from "./gate.js";
import { cleanupMaterializedRelease, loadReleaseManifest, materializeRelease, verifyMaterializedRelease, } from "./release.js";
import { computeEvidenceDigest, createSanitizedPublicationReport, } from "./report.js";
...
const EMPTY_STDERR_DIGEST = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
...
async function collectRuntimeJavaScript(root, directory = root) {
...
async function evaluatorBuildDigest() {
    const root = dirname(fileURLToPath(import.meta.url));
    return digestNamedFiles(await collectRuntimeJavaScript(root));
}
function indeterminateScenario(scenario, message, initialStateHash, durationMs) {
    return {
...
async function closeEnvironment(environment) {
    ...
    try {
        await withDeadline(environment.close(), ADAPTER_CLEANUP_TIMEOUT_MS, "adapter environment cleanup");
        return undefined;
    }
    catch (error) {
        return `adapter environment cleanup failed: ${error instanceof Error ? error.message : String(error)}`;
    }
}
async function runScenario(options) {
    ...
    const knownCandidateSecrets = knownSecretsFromCredentialEnv(Object.freeze(Object.fromEntries(options.candidateCredentialNames.map((name) => [
        name,
    ...
    try {
        ...
        const creation = options.adapter.createEnvironment(cloneJson(declaredInitialState), {
        ...
        environment =
            options.adapter.apiVersion === "agentci.adapter.v2"
                ? await creation
                : await withDeadline(creation, creationTimeoutMs, "adapter environment creation");
    }
    catch (error) {
        return indeterminateScenario(options.scenario, `fixture initialization failed: ${error instanceof Error ? error.message : String(error)}`, initialStateHash, Date.now() - startedAt);
    }
    if (remainingMs() === 0) {
        const closeIssue = await closeEnvironment(environment);
        return indeterminateScenario(options.scenario, [
            "adapter environment creation exhausted the scenario deadline",
            ...(closeIssue !== undefined ? [closeIssue] : []),
        ].join("; "), initialStateHash, Date.now() - startedAt);
    }
    try {
        const observedInitialState = await withDeadline(environment.snapshot(), Math.max(1, Math.min(remainingMs(), 5_000)), "adapter initial snapshot");
        if (!isJsonValue(observedInitialState)) {
            throw new Error("adapter returned a non-JSON initial state");
        }
        try {
            assertNoKnownSecretLeaksAtJsonBoundary(observedInitialState, knownCandidateSecrets);
        }
        catch {
            throw new Error("adapter initial state crossed the candidate credential boundary");
        }
        ...
        const closeIssue = await closeEnvironment(environment);
        return indeterminateScenario(options.scenario, [
            `initial state verification failed: ${error instanceof Error ? error.message : String(error)}`,
            ...(closeIssue !== undefined ? [closeIssue] : []),
        ].join("; "), initialStateHash, Date.now() - startedAt);
    }
    if (!Array.isArray(environment.tools) ||
        !environment.tools.every((tool) => typeof tool === "string" && tool.length > 0) ||
    ...
    try {
        ...
        const closeIssue = await closeEnvironment(environment);
        return indeterminateScenario(options.scenario, [
            `candidate driver failed unexpectedly: ${error instanceof Error ? error.message : String(error)}`,
            ...(closeIssue !== undefined ? [closeIssue] : []),
        ].join("; "), initialStateHash, Date.now() - startedAt);
    }
    let finalState;
    let finalStateHash;
    try {
        finalState = await withDeadline(environment.snapshot(), Math.max(1, Math.min(remainingMs(), 5_000)), "adapter final snapshot");
        if (!isJsonValue(finalState)) {
            throw new Error("adapter returned a non-JSON final state");
        }
        try {
            assertNoKnownSecretLeaksAtJsonBoundary(finalState, knownCandidateSecrets);
        }
        catch {
            throw new Error("adapter final state crossed the candidate credential boundary");
        }
        ...
        return {
            ...
            reasons: [
                ...driver.reasons,
                `final state snapshot failed: ${error instanceof Error ? error.message : String(error)}`,
                ...(closeIssue !== undefined ? [closeIssue] : []),
            ],
            initialStateHash,
            finalStateHash: initialStateHash,
    ...
    const stateLineageIssue = (() => {
        ...
        for (let index = 1; index < driver.events.length; index += 1) {
            if (driver.events[index - 1]?.afterStateHash !==
    ...
    if (failedAssertions.length > 0) {
        reasons.push(`${failedAssertions.length} assertion(s) failed: ${failedAssertions.map((assertion) => assertion.id).join(", ")}`);
    }
    const closeIssue = await closeEnvironment(environment);
    ...
    const verdict = driver.verdict === "block"
        ? "block"
...
async function verifyLegacyReleaseIdentity(identity) {
    try {
        ...
        return [
            `candidate entry file could not be re-verified: ${error instanceof Error ? error.message : String(error)}`,
        ];
    }
}
function capturedFilesContainKnownSecrets(files, secrets) {
    if (secrets.length === 0) {
        return false;
    }
    ...
    for (const file of files) {
        if (secrets.some((secret) => file.path.includes(secret.value)) ||
            secretBytes.some((secret) => Buffer.from(file.content).indexOf(secret) !== -1)) {
            return true;
        }
        let parsed;
        try {
...
async function executeSuite(options, includeSanitizedPublication) {
    const startedAt = Date.now();
    ...
    if (options.requireExplicitCandidatePolicy === true &&
    ...
    const candidateCredentialPolicy = resolvedRelease.capture?.manifest.schemaVersion === "agentci.release.v2"
        ? resolvedRelease.capture.manifest.candidate.credentials
        : { kind: "none" };
    if (candidateCredentialPolicy.kind === "environment" &&
        options.adapterManifestPath === undefined) {
        throw new ReleaseValidationError([
            "candidate credentials require a manifest-backed API-v2 adapter boundary",
        ]);
    }
    const manifestPreparation = options.adapterManifestPath !== undefined
    ...
    const candidateEnvironment = authorizeCandidateEnvironment({
        ...
        ...(options.approvedReleaseDigest !== undefined
            ...
            : {}),
        adapterCredentials: manifestPreparation === undefined
            ? { declaredEnvNames: [], environment: {} }
            : {
                declaredEnvNames: manifestPreparation.capture.manifest.credentials.environment,
                environment: manifestPreparation.credentials,
            },
        nonSecretRuntimeEnvironment: options.candidateRuntimeEnvironment ?? {},
    });
    const candidateCredentialNames = candidateCredentialPolicy.kind === "environment"
        ? candidateCredentialPolicy.environment
        : [];
    const candidateCredentialEnvironment = Object.freeze(Object.fromEntries(candidateCredentialNames.map((name) => [name, candidateEnvironment[name]])));
    const knownCandidateSecrets = knownSecretsFromCredentialEnv(candidateCredentialEnvironment);
    const knownAdapterSecrets = knownSecretsFromCredentialEnv(manifestPreparation?.credentials ?? {});
    const knownExecutionSecrets = [
        ...knownCandidateSecrets,
        ...knownAdapterSecrets,
    ];
    const assertNoExecutionSecretLeaks = (value) => {
        try {
    ...
    try {
        assertNoKnownSecretLeaksAtJsonBoundary(loaded.suite, knownExecutionSecrets);
        if (resolvedRelease.capture !== undefined) {
            assertNoKnownSecretLeaksAtJsonBoundary(resolvedRelease.capture.manifest, knownExecutionSecrets);
        }
    }
    catch {
        throw new ReleaseValidationError([
    ...
    if (resolvedRelease.capture !== undefined &&
        capturedFilesContainKnownSecrets(resolvedRelease.capture.files, knownExecutionSecrets)) {
        throw new ReleaseValidationError([
    ...
    const adapterIssues = await validateSuiteAgainstAdapter(loaded.suite, adapter.definition);
    ...
    for (const scenario of loaded.suite.scenarios) {
        let materialized;
        let result = indeterminateScenario(scenario, "scenario did not start", digestValue(scenario.initialState), 0);
        let canRun = true;
        if (resolvedRelease.capture !== undefined) {
            try {
                ...
                canRun = false;
                result = indeterminateScenario(scenario, `release materialization failed: ${error instanceof Error ? error.message : String(error)}`, digestValue(scenario.initialState), 0);
            }
        }
        if (canRun) {
            try {
                ...
                result = indeterminateScenario(scenario, `scenario evaluation failed unexpectedly: ${error instanceof Error ? error.message : String(error)}`, digestValue(scenario.initialState), 0);
            }
        }
        const identityIssues = [...(await adapter.verifyIdentity())];
        if (resolvedRelease.capture !== undefined && materialized !== undefined) {
            identityIssues.push(...(await verifyMaterializedRelease(materialized, resolvedRelease.capture)));
            try {
                await cleanupMaterializedRelease(materialized);
            }
            catch (error) {
                identityIssues.push(`materialized release cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    ...
    try {
        assertNoKnownSecretLeaksAtJsonBoundary(report, knownExecutionSecrets);
    }
    catch {
        throw new ReleaseValidationError([
    ...
    try {
        assertNoKnownSecretLeaksAtJsonBoundary(publication, knownExecutionSecrets);
    }
    catch {
        throw new ReleaseValidationError([
...
export async function runSuiteWithEvidenceGuard(options) {
...
export async function runSuiteWithSanitizedPublication(options) {
