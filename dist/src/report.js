...
function stableScenario(scenario) {
    const { durationMs: _durationMs, events, ...stable } = scenario;
    return { ...stable, events: events.map(stableEvent) };
}
export function computeEvidenceDigest(report) {
    const release = report.release.digestScope === "entry-file-only"
        ? {
    ...
    return digestValue({
        ...
        release,
        adapter: report.adapter.digestScope === "module-entry-only"
            ? {
            ...
            : {
                ...
                credentialDeclarationDigest: report.adapter.credentialDeclarationDigest,
...
export function verifyEvidenceDigest(report) {
...
export function computePublicationDigest(report) {
    return digestValue(report);
}
export function createSanitizedPublicationReport(report) {
    ...
    const adapter = report.adapter.digestScope === "module-entry-only"
        ? {
            digestScope: report.adapter.digestScope,
            moduleDigest: report.adapter.moduleDigest,
        }
    ...
    const withoutDigest = {
        ...
        scenarios: report.scenarios.map((scenario, index) => ({
            ...
            candidateStderrTruncated: scenario.candidateDiagnostics.stderrTruncated,
        })),
        decision: {
...
export function verifyPublicationDigest(report) {
...
export function isSanitizedPublicationReport(value) {
    if (!isRecord(value) ||
        !hasOnlyKeys(value, [
            "schemaVersion",
            "profile",
    ...
    if (!isRecord(decision) ||
        ...
        decision.passed !==
            scenarios.filter((scenario) => isRecord(scenario) && scenario.verdict === "pass").length ||
        decision.blocked !==
            scenarios.filter((scenario) => isRecord(scenario) && scenario.verdict === "block").length ||
        decision.indeterminate !==
            scenarios.filter((scenario) => isRecord(scenario) && scenario.verdict === "indeterminate").length ||
        decision.verdict !==
            (decision.blocked > 0
                ? "block"
    ...
    if (!isRecord(publication) ||
        ...
        publication.omitted.some((item, index) => item !== SANITIZED_PUBLICATION_OMISSIONS[index])) {
        return false;
    }
    return true;
}
function isReleaseModel(value) {
    if (!isRecord(value)) {
        return false;
    }
    if (value.kind === "none") {
...
function isManifestReleaseIdentity(value) {
    ...
    if (identities.length !== files.length ||
        identities.length !== value.fileCount ||
        identities.some((file) => typeof file.path !== "string" ||
        ...
        identities.some((file, index) => index > 0 &&
            typeof identities[index - 1]?.path === "string" &&
            identities[index - 1].path >= file.path)) {
        return false;
    }
    const fileRecords = identities;
    ...
    return (value.manifestDigest === manifestDigest &&
...
function normalizeAdapterManifest(value) {
    try {
...
function isManifestAdapterIdentity(value) {
    ...
    const fileRecords = identities;
    const entry = fileRecords.find((file) => file.path === parsedManifest.runtime.entry);
    if (value.id !== parsedManifest.id ||
...
function isAdapterIdentity(value) {
    if (!isRecord(value)) {
        return false;
    }
...
function hasConsistentStateLineage(value) {
...
function isScenarioResult(value) {
    if (!isRecord(value)) {
        return false;
    }
    return (hasOnlyKeys(value, [
        ...
        value.events.every((event, index) => isToolEvent(event) &&
        ...
        (value.verdict !== "pass" ||
            (value.reasons.length === 0 &&
                ...
                value.assertions.every((assertion) => isRecord(assertion) && assertion.passed === true))) &&
        ...
        value.candidateDiagnostics.stderrBytes >= 0 &&
        typeof value.candidateDiagnostics.stderrTruncated === "boolean" &&
        value.candidateDiagnostics.stderrTruncated ===
            (value.candidateDiagnostics.stderrBytes >
                CANDIDATE_STDERR_CAPTURE_LIMIT_BYTES) &&
        typeof value.durationMs === "number" &&
        Number.isFinite(value.durationMs) &&
        value.durationMs >= 0);
}
function isGateDecision(value, scenarios) {
    ...
    const expectedVerdict = blocked > 0 ? "block" : indeterminate > 0 ? "indeterminate" : "pass";
    return (hasOnlyKeys(value, [
        "verdict",
        "reasons",
        "passed",
...
export function isReleaseReport(value) {
    if (!isRecord(value)) {
        return false;
    }
    const suite = value.suite;
    ...
    return (hasOnlyKeys(value, [
        ...
        isGateDecision(value.decision, scenarios) &&
        isDigest(value.evidenceDigest));
}
export async function writeTextFile(path, content) {
    assertSecretScanClean(scanTextForSecrets({ path: "evidence/output.txt", text: content }), "evidence publication");
...
export async function writeSanitizedPublicationReport(path, report) {
...
export function renderMarkdownReport(report) {
    const releaseLines = report.release.digestScope === "entry-file-only"
        ? [
    ...
    const adapterLines = report.adapter.digestScope === "module-entry-only"
        ? [
    ...
    for (const scenario of report.scenarios) {
        const assertionPasses = scenario.assertions.filter((assertion) => assertion.passed).length;
        lines.push(`| ${markdownText(scenario.scenarioId)} | ${verdictIcon(scenario.verdict)} | ${assertionPasses}/${scenario.assertions.length} | ${scenario.events.length} |`);
    }
    for (const scenario of report.scenarios.filter((candidate) => candidate.verdict !== "pass")) {
        lines.push("", `## ${markdownText(scenario.scenarioId)}: ${verdictIcon(scenario.verdict)}`, "");
        ...
        for (const assertion of scenario.assertions.filter((candidate) => !candidate.passed)) {
            lines.push(`- Assertion ${markdownCode(assertion.id)}: ${markdownText(assertion.message)}`);
        }
    }
    lines.push("", "> This report evaluates only the declared suite, captured release bytes, declared adapter bytes/configuration, and observed adapter state. It does not establish remote-system fidelity, guarantee remote cancellation or rollback, or certify production safety.", "");
    return lines.join("\n");
}
export function renderSanitizedPublicationMarkdown(report) {
    if (!isSanitizedPublicationReport(report) || !verifyPublicationDigest(report)) {
        throw new Error("sanitized publication is invalid or its digest does not match");
    }
    const releaseDigest = report.release.digestScope === "entry-file-only"
        ? report.release.entryFileDigest
        : report.release.releaseDigest;
    const adapterDigest = report.adapter.digestScope === "module-entry-only"
    ...
    for (const scenario of report.scenarios) {
        lines.push(`| ${scenario.scenario} | ${verdictIcon(scenario.verdict)} | ${scenario.assertionsPassed}/${scenario.assertionsTotal} | ${scenario.toolCallCount} |`);
    }
    lines.push("", `> This sanitized publication omits logical identifiers, timestamps/durations, scenario descriptions, tasks, state, tool arguments/results, outputs, assertion details, reasons, manifests, file inventories, local paths, and candidate-stderr digests. Full evidence was not published. Recommended retention is ${report.publication.recommendedRetentionDays} days; the caller must configure and enforce deletion for every retained copy, including artifacts and job summaries.`, "");
    return lines.join("\n");
}
export function renderSanitizedConsoleSummary(report) {
    if (!isSanitizedPublicationReport(report) || !verifyPublicationDigest(report)) {
        throw new Error("sanitized publication is invalid or its digest does not match");
    }
    return [
...
export function renderConsoleSummary(report) {
    const rows = report.scenarios.map((scenario) => `${scenario.verdict.toUpperCase().padEnd(13)} ${scenario.scenarioId} (${scenario.assertions.filter((assertion) => assertion.passed).length}/${scenario.assertions.length} assertions)`);
    return [
