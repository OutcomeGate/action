...
export function readJsonPointer(root, pointer) {
    if (root === undefined) {
        return MISSING;
    }
    if (pointer === "") {
...
function baseResult(assertion, passed, message, expected, observed) {
    return {
        id: assertion.id,
...
export function evaluateAssertion(assertion, context) {
    if (assertion.type === "json_pointer") {
        ...
        if (assertion.operator === "absent") {
            ...
            return baseResult(assertion, passed, passed
                ? `${assertion.source}${assertion.pointer} is absent`
                : `${assertion.source}${assertion.pointer} was expected to be absent`, undefined, observed === MISSING ? undefined : observed);
        }
        const passed = observed !== MISSING &&
            stableStringify(observed) === stableStringify(assertion.expected);
        return baseResult(assertion, passed, passed
            ? `${assertion.source}${assertion.pointer} equals the expected value`
            : `${assertion.source}${assertion.pointer} differs from the expected value`, assertion.expected, observed === MISSING ? undefined : observed);
    }
    if (assertion.type === "event_count") {
        const outcome = assertion.outcome ?? "any";
        const observed = context.events.filter((event) => event.tool === assertion.tool &&
            (outcome === "any" || event.outcome === outcome)).length;
        const passed = observed === assertion.expected;
        return baseResult(assertion, passed, passed
            ? `${assertion.tool} call count is ${observed}`
            : `${assertion.tool} call count was ${observed}; expected ${assertion.expected}`, assertion.expected, observed);
    }
    ...
    for (const tool of assertion.tools) {
        ...
        if (foundAt === -1) {
            return baseResult(assertion, false, `required order was not observed: ${assertion.tools.join(" -> ")}`, assertion.tools, eventTools);
        }
        cursor = foundAt + 1;
    }
    return baseResult(assertion, true, `required order was observed: ${assertion.tools.join(" -> ")}`, assertion.tools, eventTools);
}
