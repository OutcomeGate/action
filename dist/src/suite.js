...
function readPositiveInteger(value, path, issues) {
...
function parseFault(value, path, issues) {
    if (!isRecord(value)) {
...
function parseAssertion(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return undefined;
    }
    ...
    if (value.type === "json_pointer") {
        unknownKeys(value, ["id", "description", "type", "source", "pointer", "operator", "expected"], path, issues);
    ...
    if (value.type === "event_count") {
        unknownKeys(value, ["id", "description", "type", "tool", "outcome", "expected"], path, issues);
        const tool = readString(value.tool, `${path}.tool`, issues);
        ...
        if (id === undefined || tool === undefined || !Number.isInteger(expected) || expected < 0 || (outcome !== "any" && outcome !== "ok" && outcome !== "error")) {
            return undefined;
        }
        return {
            id,
...
function parseScenario(value, index, issues) {
...
export function parseSuite(value) {
    const issues = [];
    if (!isRecord(value)) {
        throw new SuiteValidationError(["suite must be an object"]);
    }
    unknownKeys(value, ["schemaVersion", "name", "version", "fixture", "gate", "scenarios"], "", issues);
    ...
    if (!isRecord(value.gate)) {
        ...
        if (typeof value.gate.minPassRate !== "number" ||
    ...
    const fixtureIssues = suite.fixture === "refunds.v1" ? validateSuiteForFixture(suite) : [];
...
export async function loadSuite(path) {
    ...
    assertSecretScanClean(scanTextForSecrets({ path: "suite/input.json", text: raw }), "suite input");
    let parsed;
    try {
