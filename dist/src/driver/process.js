...
import { assertNoKnownSecretLeaks, assertNoKnownSecretLeaksAtJsonBoundary, knownSecretsFromCredentialEnv, } from "../credential-policy.js";
...
function parseCandidateMessage(value) {
...
function serializeCandidateInput(message) {
    return `${JSON.stringify(message)}\n`;
}
function matchingFault(faults, tool, onCall) {
    return faults.find((fault) => fault.tool === tool && fault.onCall === onCall);
}
function expectedToolError(value) {
    if (value instanceof ToolCallError) {
        return value.detail;
    }
    if (value !== null &&
        typeof value === "object" &&
        value.agentciToolError === true &&
...
async function snapshotDigest(environment, knownCandidateSecrets) {
    ...
    try {
        ...
        throw new FixtureError("adapter state crossed the candidate credential boundary");
    }
...
export async function runCandidateProcess(options) {
    ...
    const candidateEnvironment = options.candidateEnvironment ?? Object.freeze({});
    const candidateCredentialEnvironment = Object.freeze(Object.fromEntries((options.candidateCredentialNames ?? Object.keys(candidateEnvironment)).map((name) => [name, candidateEnvironment[name]])));
    const knownCandidateSecrets = knownSecretsFromCredentialEnv(candidateCredentialEnvironment);
    const knownExecutionSecrets = options.knownExecutionSecrets ?? knownCandidateSecrets;
    try {
        assertNoKnownSecretLeaksAtJsonBoundary(scenario.task, knownCandidateSecrets);
    }
    catch {
        throw new FixtureError("scenario task crossed the candidate credential boundary");
    }
    const candidateWorkingDirectory = dirname(candidatePath);
    const candidateCredentialNameSet = new Set(options.candidateCredentialNames ?? Object.keys(candidateEnvironment));
    const nonCredentialEnvironment = Object.fromEntries(Object.entries(candidateEnvironment).filter(([name]) => !candidateCredentialNameSet.has(name)));
    try {
        ...
        throw new FixtureError("candidate spawn metadata crossed a known credential boundary");
    }
    ...
    let forceKillTimer;
    const candidateCredentialBytes = Object.values(candidateCredentialEnvironment).map((value) => Buffer.from(value, "utf8"));
    const longestCandidateCredential = candidateCredentialBytes.reduce((maximum, value) => Math.max(maximum, value.byteLength), 0);
    let candidateCredentialStderrTail = Buffer.alloc(0);
    const protocolCredentialBytes = knownExecutionSecrets.map((secret) => Buffer.from(secret.value, "utf8"));
    const longestProtocolCredential = protocolCredentialBytes.reduce((maximum, value) => Math.max(maximum, value.byteLength), 0);
    ...
    const sendToCandidate = (message) => {
        ...
        if (protocolCredentialBytes.some((credential) => combined.indexOf(credential) !== -1)) {
            throw new FixtureError("runner-to-candidate protocol crossed a known credential boundary");
        }
        candidateInputCredentialTail = combined.subarray(Math.max(0, combined.byteLength - Math.max(0, longestProtocolCredential - 1)));
    ...
    child.stdout.on("data", (chunk) => {
        ...
        if (protocolCredentialBytes.some((credential) => combined.indexOf(credential) !== -1)) {
            ...
            if (environment.abort !== undefined) {
                void Promise.resolve(environment.abort("candidate stdout credential boundary violation")).catch(() => undefined);
            }
            stopCandidate();
            return;
        }
        candidateStdoutCredentialTail = combined.subarray(Math.max(0, combined.byteLength - Math.max(0, longestProtocolCredential - 1)));
    });
    ...
    child.stderr.on("data", (chunk) => {
        ...
        if (!candidateStderrSecretLeak) {
            try {
                ...
                if (candidateCredentialBytes.some((credential) => combined.indexOf(credential) !== -1)) {
                    throw new Error("known candidate credential detected");
                }
                candidateCredentialStderrTail = combined.subarray(Math.max(0, combined.byteLength - Math.max(0, longestCandidateCredential - 1)));
                ...
                candidateFailure = true;
                preserveKnownCandidateBlock = true;
                reasons.push("candidate stderr crossed a known credential boundary");
                if (environment.abort !== undefined) {
                    void Promise.resolve(environment.abort("candidate stderr credential boundary violation")).catch(() => undefined);
                }
                stopCandidate();
            }
        }
    ...
    const handleToolCall = async (message) => {
        ...
        if (environment.transition !== undefined) {
            ...
            try {
                const transition = await environment.transition(invoke
                    ? {
                        invoke: true,
                        tool: message.tool,
                    ...
                    : { invoke: false });
                if (!isJsonValue(transition.beforeState) ||
                    !isJsonValue(transition.afterState)) {
                    runnerFailure = "adapter transition returned a non-JSON state";
                }
                if (runnerFailure === undefined) {
                    try {
                        assertNoKnownSecretLeaksAtJsonBoundary(transition.beforeState, knownCandidateSecrets);
                        assertNoKnownSecretLeaksAtJsonBoundary(transition.afterState, knownCandidateSecrets);
                ...
                if (!invoke) {
                    ...
                    if (beforeStateHash !== afterStateHash) {
                        ...
                        if (preserveKnownCandidateBlock) {
                            reasons.push(`adapter integrity concern after candidate block: ${issue}`);
                        }
                        else {
                            runnerFailure = issue;
                        }
                    ...
                    if (typeof transition.outcome.error?.code !== "string" ||
            ...
            if (!unknownTool && fault?.phase !== "before") {
                ...
                try {
                    ...
                    if (expected !== undefined) {
                        ...
                        runnerFailure = `fixture threw unexpectedly: ${caught instanceof Error ? caught.message : String(caught)}`;
                    }
                }
                if (fault?.phase === "after" &&
        ...
        if (runnerFailure === undefined) {
            try {
                if (content !== undefined) {
                    assertNoKnownSecretLeaksAtJsonBoundary(content, knownCandidateSecrets);
                }
                if (error !== undefined) {
                    assertNoKnownSecretLeaksAtJsonBoundary({ code: error.code, message: error.message }, knownCandidateSecrets);
                }
            }
            catch {
                content = undefined;
    ...
    const handleLine = async (line) => {
        ...
        try {
            ...
            candidateFailure = true;
            preserveKnownCandidateBlock = true;
            reasons.push("candidate stdout protocol crossed a known credential boundary");
            if (environment.abort !== undefined) {
                void Promise.resolve(environment.abort("candidate stdout credential boundary violation")).catch(() => undefined);
            }
            stopCandidate();
        ...
        try {
            ...
            if (environment.abort !== undefined) {
                void Promise.resolve(environment.abort("candidate protocol credential boundary violation")).catch(() => undefined);
            }
            stopCandidate();
            return;
        }
        if (isJsonValue(raw) && environment.inspectCandidateProtocol !== undefined) {
            try {
                ...
                candidateFailure = true;
                preserveKnownCandidateBlock = true;
                reasons.push("candidate protocol crossed the known adapter-credential boundary");
                if (environment.abort !== undefined) {
                    void Promise.resolve(environment.abort("candidate protocol credential boundary violation")).catch(() => undefined);
                }
                stopCandidate();
    ...
    lines.on("line", (line) => {
        work = work.then(() => handleLine(line)).catch((error) => {
            const message = `runner could not process candidate message: ${error instanceof Error ? error.message : String(error)}`;
            if (preserveKnownCandidateBlock) {
    ...
    const timeout = setTimeout(() => {
        ...
        if (activeAdapterOperation) {
            if (candidateFailure) {
                preserveKnownCandidateBlock = true;
                reasons.push("adapter transition was terminated after an independently established candidate defect");
            }
            else {
                adapterCommitUnknown = true;
                reasons.push("scenario deadline expired during an adapter transition; commit state is unknown");
            }
            if (environment.abort !== undefined) {
                void Promise.resolve(environment.abort("scenario deadline expired during adapter transition")).catch(() => undefined);
            }
        }
        else {
            candidateFailure = true;
    ...
    const workFinished = await Promise.race([
        work.then(() => true),
        new Promise((resolveWorkDeadline) => {
    ...
    if (!workFinished) {
        if (preserveKnownCandidateBlock) {
            reasons.push("adapter/tool work did not settle after the independently established candidate defect");
        }
        else {
