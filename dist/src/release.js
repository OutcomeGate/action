...
function parseStringPaths(value, path, issues) {
...
function parseModel(value, issues) {
    if (!isRecord(value)) {
    ...
    if (value.kind === "remote") {
        unknownKeys(value, ["kind", "provider", "identifier", "revision", "configuration"], "model", issues);
...
export function parseReleaseManifest(value) {
    ...
    if (schemaVersion !== "agentci.release.v1" &&
    ...
    if (schemaVersion === "agentci.release.v2") {
        if (!isRecord(value.candidate)) {
            ...
            try {
                candidate = {
                    credentials: parseCandidateCredentialPolicy(value.candidate.credentials),
                };
            }
            catch (error) {
                issues.push(`candidate.credentials is invalid: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    let components;
    if (!isRecord(value.components)) {
        ...
        const toolSchemas = parseStringPaths(value.components.toolSchemas, "components.toolSchemas", issues);
...
export async function loadReleaseManifest(path) {
    ...
    try {
        ...
        assertSecretScanClean(scanTextForSecrets({ path: "release/manifest.json", text: raw }), "release manifest");
    ...
    if (normalizedJsonFiles.length > 0) {
        assertSecretScanClean(scanNamedArtifactsForSecrets(normalizedJsonFiles), "normalized release JSON bundle files");
    }
    ...
    const toolSchemas = manifest.components.toolSchemas.map((schemaPath) => byPath.get(schemaPath));
    ...
    const toolSchemaDigest = computeBundleDigest("agentci.tool-schema-set.v1", toolSchemas);
...
export async function materializeRelease(capture) {
...
export async function verifyMaterializedRelease(materialized, capture) {
    try {
        ...
        return [
            `materialized release bundle could not be verified: ${error instanceof Error ? error.message : String(error)}`,
        ];
    }
}
export async function cleanupMaterializedRelease(materialized) {
